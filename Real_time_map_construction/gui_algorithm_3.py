...
class Gui:
    def __init__(self,screen_size):
        ...
        pass
    def draw_cube(self,map,ycoord,xcoord,margin=5,block_size=20,color=(100,100,255)):
    ...
    def draw_map(self,maze_format,screen_size=500,block_size=25,wall_color=30,end_color
        ...
        El block_size debe ser divisor de self.screen_size
        """
        temp_map=np.zeros((self.screen_size,self.screen_size),np.uint8)
        map=cv2.cvtColor(temp_map,cv2.COLOR_GRAY2BGR)
        if(not(self.screen_size%block_size==0)):
        ...
        """
        cv2.putText(map,corner_text,(0,self.screen_size-block_size//2),font,corner_text_size
